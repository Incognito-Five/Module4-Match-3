...
function Board:initializeTiles()
    ...
    for tileY = 1, 8 do
        ...
        for tileX = 1, 8 do
            ...
            -- to start with simple flat blocks
            table.insert(self.tiles[tileY], Tile(tileX, tileY, math.random(maxColorNum), math.random(math.min(level, 6))))
        end
    end
...
--[[
    Resets the board if there isn't possible matches
]]
    ...
    for x = 1, 8 do
        for y = 8, 1, -1 do
            ...
            if not tile then
                ...
                local tile = Tile(x, y, math.random(maxColorNum), math.random(math.min(level, 6)))
                tile.y = -32
                self.tiles[y][x] = tile
